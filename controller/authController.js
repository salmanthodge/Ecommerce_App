...
export const forgotPasswordController = async (req, res) => {
  try {
    ...
    if (!email) {
      res.status(400).send({ messege: "Email is require" });
    }
    if (!answer) {
      res.status(400).send({ messege: "answer is require" });
    }
    if (!newPassword) {
      res.status(400).send({ messege: "New Password is require" });
    }
    //check
...
export const updateProfilecontroller = async (req, res) => {
  try {
    ...
    res.status(400),
      send({
        success: false,
        messege: "Error while updating profile",
        error,
      });
  }
};
